...
class RespHandlerTest {
    ...
    private RespResult process(String input) throws IOException {
        return respHandler.processCommand(createInput(input));
    }

    @Test
    @DisplayName("PING 명령어에 대해 +PONG\\r\\n 응답")
    void pingCommand() throws IOException {
        RespResult result = process("PING\r\n");

        assertThat(result.response()).isEqualTo("+PONG\r\n");
    ...
    void respArraySetAndGet() throws IOException {
        ...
        String setInput = "*3\r\n$3\r\nSET\r\n$4\r\nkey1\r\n$6\r\nvalue1\r\n";
        RespResult setResponse = process(setInput);
        assertThat(setResponse.response()).isEqualTo("+OK\r\n");

        // GET key1 (RESP Array)
        String getInput = "*2\r\n$3\r\nGET\r\n$4\r\nkey1\r\n";
        RespResult getResponse = process(getInput);
        assertThat(getResponse.response()).isEqualTo("$6\r\nvalue1\r\n");
    }
    ...
    void getNullKey() throws IOException {
        ...
        assertThat(getResponse.response()).isEqualTo("$-1\r\n");
    }
    ...
    void utf8BulkString() throws IOException {
        ...
        assertThat(process(setInput).response()).isEqualTo("+OK\r\n");
        assertThat(process("GET key\r\n").response())
                .isEqualTo("$6\r\n안녕\r\n");
    }
    ...
    void truncatedBulkString() throws IOException {
        RespResult result = process("*1\r\n$4\r\nGET\r\n");

        assertThat(result.response()).startsWith("-ERR Protocol error:");
    ...
    void missingBulkLengthPrefix() throws IOException {
        ...
        assertThat(result.response()).isEqualTo("-ERR Protocol error: expected bulk string\r\n");
        assertThat(result.closeConnection()).isTrue();
    }
    ...
    void invalidArrayLength() throws IOException {
        RespResult result = process("*abc\r\n");

        assertThat(result.response()).isEqualTo("-ERR Protocol error: invalid array length\r\n");
        assertThat(result.closeConnection()).isTrue();
    }
